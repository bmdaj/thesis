...
As shown in \secref{sec:topopt_theory} for $N$ with \textbf{weak nonlinear coupling} (\eqref{eq:multiphysics_weak_nonlinear}) the total system of equations will be upper or lower triangular
...
\left\{
    \begin{aligned}
        \quad \mathbf{K}_1(\mathbf{u}_2, \ldots, \mathbf{u}_N)\, \mathbf{u}_1  + \sum_{j=2}^N \mathbf{C}_{1 \, j} (\mathbf{u}_2, \cdots \mathbf{u}_N)\, \mathbf{u}_j &= \mathbf{f}_1\,, \\
        \quad\mathbf{K}_2(\mathbf{u}_3, \ldots, \mathbf{u}_N)\, \mathbf{u}_2 + \sum_{j=3}^N \mathbf{C}_{2 \, j} (\mathbf{u}_3, \cdots \mathbf{u}_N)\, \mathbf{u}_j &= \mathbf{f}_2\,, \\
        \quad\ldots \\
...
To do the adjoint sensitivity analysis we start by rewriting the FOM by adding the residual
...
\end{equation}
Now we calculate the sensitivities by taking the derivative of the FOM ($\Psi$) with respect to the physical field ($\hat{\rho}$)
\begin{align}\label{eq:sens_init}
    ...
    + \sum_{i=1}^N \Lambda_i^\top \Bigg\{\Bigg(
        ...
        &\quad + \sum_{j=i+1}^{N} \Bigg[ \Bigg(
            \frac{\partial C_{ij}}{\partial \hat{\rho}} + 
...
\end{align}
This chain-rule-based derivative expression takes into account all the dependencies of the solutions in the weakly coupled problem.
...
\end{equation}
This means that the adjoint equations are solved in recevere order with respect to the forward solution (\eqref{eq:app_multiphysics_weak}). In other words, one needs to solve the first adjoint equation ($i=1$) 
