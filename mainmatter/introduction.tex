...
density\footnote{Note that this \emph{density} is not a density in the physical sense of the word but rather a design field used in topology optimization.} field $\rho(\mathbf{r}) \in [0,1]$, where $\mathbf{r}$ is the position vector, $\rho = 0$ corresponds to one material, typically air or void, and $\rho = 1$ corresponds to a solid material.
 As illustrated 
