...
thermo-optic (\chapref{chap:to})~\cite{TOPS_1, TOPS_2, TOPS_3, program, PIC} and electro-optic (\chapref{chap:eo})~\cite{modu, modu1, modu2, pockels} effects, to nanolasers (\secref{sec:laser}) that balance optical gain, carrier dynamics, and heat dissipation~\cite{laser,laser_pic}, multiphysics couplings enable stable, efficient on-chip light manipulation. Equally, sensors  
...
Precise optomechanical manipulation (\chapref{chap:om}), whether trapping nanoparticles (\secref{sec:dip})~\cite{ashkin_acceleration_1970} or actuating tiny structures with light (\secref{sec:mech_strongly_coupled})~\cite{ivanyi_optically_2024}, further illustrates how forces and electromagnetic fields work together at the nanoscale. Quantum photonic devices also
