...
\section{Nanophotonics -- light as an electromagnetic
  ...
  be described using different physical models. As shown in \figref{fig:EM_regime},
...
\end{equation}
where $\varepsilon^\prime$ is the real part of the permittivity, $\text{i}$ is the imaginary unit, $\varepsilon^{\prime\prime}$ is the imaginary part of the permittivity, $\underbar{n}$ is the complex refractive index, 
...
in the formulation in \cite{ownpub2}).
    ...
    the interfaces. These boundary conditions are derived by applying Gauss's theorem to
     the divergence equations \eqref{eq:divD_freq} and \eqref{eq:divB_freq}, and Stokes's
      theorem to the curl equations \eqref{eq:curlE_freq} and \eqref{eq:curlH_freq} of
       Maxwell's equations~\cite{novotny}. This ensures field continuity
...
%    \end{figure}
 ...
 across the element boundaries, as we derived in~\eqref{eq:BC_E}. Using the shape functions ($\boldsymbol{N}$) of the Nédélec elements,
 ...
 domain in~\figref{fig:top_opt} into a set of triangular Nédélec elements, where we visualize the shape function for one of the edges.
 ...
 one can discretize the weak form
 into the matrix form, which gives a linear system of equations:
 ...
 vector $\mathbf{F}$ containing the degrees of freedom of the forcing term. As mentioned previosly (see discussion on hermiticity in \secref{sec:nanophotonics}), in the general case where there are optical losses [in materials ($\varepsilon^{\prime \prime} \neq 0$), or boundary conditions] the stiffness matrix is a sparse symmetric indefinite matrix \footnote{See discussion on hermiticity
 ...
 the adjoint problem (e.g., \eqref{eq:adjoint_eqs} in~\secref{sec:topopt_theory}), without solving an extra linear system of equations. To reduce the computational cost of solving the system of equations in~
 ...
 the temperature-dependent refractive index. Thus, we first solve a heat conduction problem to find the temperature distribution. 
 ...
 equation $\mathcal{L}_2 [\mathbf{u}_2]= \mathbf{f}_2$. Using the finite-element method (\secref{sec:fem}), these equations can be
...
$\mathbf{C}_{21}$ is the linear term that couples physics $1$ to physics $2$, and the
 ...
 depend on the solution vector, and all couplings are
...
\subsection*{$N$ coupled linear problems}
 ...
 \mathbf{f}_N
        \end{bmatrix}\,.
    \end{equation}
 where we have dropped the prime notation for convenience. In this case, the
 definition of weak and strong coupling
 becomes more complex since two physics might be coupled indirectly through other
 physics. Nevertheless, following a similar logic as in the $N=2$ case, if
 the coupled
 ...
 nonlinear. In this thesis, we consider the interesting case of \textbf{weak nonlinear coupling},
 which is common in many
 coupled multiphysics problems. As an example of such a system, we consider the problem in \secref{sec:TOPS}~\cite{ownpub0}, where the refractive index encoded in 
 the stiffness matrix depends on the solution of the temperature field.
 In these types of problems, the system of equations is based on an upper or lower triangular matrix, where the stiffness matrices of the coupled 
 ...
 Note that when there are non-zero entries in the lower triangle of the matrix, or the dependence on the
 ...
 is repeated until a convergence criterion is met (e.g., \secref{sec:mech_strongly_coupled}~\cite{ownpub5}). Note that in weakly coupled systems, the solution of one part feeds directly into the next. 
 ...
 topology optimization, we refer the reader to the book chapter by Maute~\cite{coupled_topopt}.
    ...
    \begin{equation}
        ...
        \end{array}
    \end{equation}
    where the FOM ($\Psi$) is maximizing for a design field $\rho$ with upper
   ...
   $h^\text{th}$ element. One can also filter by solving a Helmholtz-type
    ...
    respectively. Note that in topology optimization problems the threshold sharpness ($\beta$) is gradually increased as the optimization progresses, which is known as 
    a continuation scheme. This ensures that
 ...
 For example, the well-known Simplified Isotropic Material with Penalization (SIMP) material interpolation~\cite{SIMP} can be used:
    \begin{equation}
 ...
 for other problems where a linear interpolation suffices. For an example, see the use of it in \cite{ownpub3,ownpub4}.}. Note that for some
 ...
 to employ a non-linear material interpolation~\cite{nonlinear_interp,ownpub0}.
    ...
    \textbf{Adjoint sensitivity analysis} provides an efficient way to calculate the gradient of the FOM with respect to the design variables,
 ...
 nonlinear coupling (\eqref{eq:multiphysics_weak_nonlinear}) solved using a segregated approach, with the detailed derivations in 
    \appref{app:appendix1}. Note that for problems solved with a monolithic approach, where all physics are handled
 in a single solve, the adjoint sensitivities can be computed directly by solving one adjoint problem for the entire
 ...
 \frac{\d \Psi}{\d \rho} = \frac{\partial \Psi}{\partial \hat{\rho}} \cdot \frac{\partial \hat{\rho}}{\partial \tilde{\rho}} \cdot \frac{\partial \tilde{\rho}}{\partial \rho}\,,
    \end{equation}
 where the final sensitivity $\d \Phi/\d \rho$ are the sensitivities used by the optimization algorithm.
