...
\begin{align}
    \bm{\mathcal{D}}(\mathbf{r}, t) & = \varepsilon_0 \varepsilon \bm{\mathcal{E}}(\mathbf{r}, t) =
    \bm{\mathcal{E}}(\mathbf{r}, t) + \bm{\mathcal{P}}(\mathbf{r}, t),
    \label{eq:D}                                                                \\
    \bm{\mathcal{B}}(\mathbf{r}, t) & =  \mu \bm{\mathcal{H}}(\mathbf{r},
    t) = \mu_0 \bm{\mathcal{H}}(\mathbf{r},
    t) + \bm{\mathcal{M}}(\mathbf{r}, t) \label{eq:H},
\end{align}
EXPLAIN EPSILON AND MU AND REFRACTIVE INDEx where $\varepsilon_0$ is the permittivity of free space and $\mu_0$ is the
permeability of free space, and we have considered non-dispersive media.
...
$i$ from domain $j$, the tangential field conditions are~\cite{novotny}:
    ...
    equations. In \secref{sec:fem}, we show how discretizing the domain with
    Nédélec elements,
...
$2$, but not the other way around.
    ...
    In the case of $N$ coupled problems, the system of equations becomes:
    ...
    In general, there are two possibilities for solving coupled problems. If it is
    ...
    Still today this is an active topic of research, where state-of-the-art multiphysics topology optimization is used tackle complex coupled  design problems; for instance, optimizing for thermo-mechanical~\cite{third_thermal}, magneto-mechanical~\cite{magneto}, or fluid-structure interactions. 
    ...
    In the topology optimization of multiphysics systems we consider a general constrained optimization problem 
    expressed as:
    \begin{equation}
        \begin{array}{clr}
...
$h^\text{th}$ element. One can also filter by solving a Helmholtz-type
    ...
    \figref{fig:ft} we show the effects of filtering and thresholding on the design field for a discretized circle.
    ...
    NOTE ON CONTINUATION SCHEME HERE
    ...
    different physical problem\footnote{If the problem stem from the same physics
        ...
        re-used.}.
    For example, the SIMP material interpolation (CITE) can be used:
    \begin{equation}
