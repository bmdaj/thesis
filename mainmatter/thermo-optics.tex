...
Using this configuration and perfect electric conductor boundary conditions (\secref{sec:nanophotonics}), we solve the eigenproblem in \eqref{eq:wg_eq} at the telecom wavelength ($\lambda = 1.55$ \textmu m) for varying extinction coefficients. 
 As shown in \figref{fig:pt}, increasing the extinction coefficient initially makes the fundamental mode more lossy until a bifurcation splits
