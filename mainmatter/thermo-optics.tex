...
Using this configuration and PEC boundary conditions (\secref{sec:nanophotonics}), we solve the generalized eigenvalue
 problem \eqref{eq:wg_eq} at the telecom wavelength ($\lambda = 1.55$ \textmu m) for varying extinction coefficients. 
 As shown in \figref{fig:pt}, increasing loss initially makes the fundamental mode more lossy until a bifurcation splits
...
Using this framework, we find the results in \figref{fig:thermo_res} (b), where we depict the waveguide cross-section with the topology-optimized metallic heater,
