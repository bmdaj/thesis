...
oscillating, and trapping particles. In \figref{fig:eng_res}, we show an example optimization result from this work, where we optimize the geometry of a particle to maximize the vertical component of the optical force. 
...
field and the forcing term, respectively; $\mathbf{R}$ is the residual of the
 ...
 \mathbf{f}_\text{MST} = \hat{\rho} \cdot \nabla \cdot \langle\overleftrightarrow{\bm{\mathcal{T}}}(\mathbf{r}, t)\rangle \,,
    \end{equation}
 which can be integrated over the volume of the simulation domain to find the total force. Lastly, to ensure structural integrity, we apply a connectivity constraint via the VTM~\cite{li_structural_2016} (\secref{sec:aux}), which ensures that the design is connected to membrane ends. 
